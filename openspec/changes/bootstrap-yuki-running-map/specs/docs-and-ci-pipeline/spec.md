...
### Requirement: Vercel Preview Deployment is wired up for every PR
The system SHALL connect the repo to a Vercel project so that every pull request gets an isolated Preview Deployment whose root and all five placeholder routes return HTTP 200.

#### Scenario: PR Preview URL returns 200 for placeholders
- **WHEN** a PR is pushed to GitHub
- **THEN** Vercel posts a Preview Deployment URL
- **AND** GET `/`, `/routes`, `/routes/example-route`, `/admin/login` each return HTTP 200 on the preview
- **AND** unauthenticated GET `/admin/upload` returns a 307 redirect to `/admin/login`
- **AND** required env vars (Supabase keys, ADMIN_GITHUB_USERNAME, NEXT_PUBLIC_PMTILES_URL) are configured for both Production and Preview environments

### Requirement: Playwright smoke tests cover the five placeholder routes
The system SHALL include a Playwright E2E test suite (`pnpm test:e2e`) covering five scenarios — visitor opens each of the four public placeholder routes, and unauthenticated visitor to `/admin/upload` is redirected — all passing locally and in CI.

#### Scenario: Suite passes locally
- **WHEN** a developer runs `pnpm test:e2e` on a clean checkout
- **THEN** all five Playwright tests pass
- **AND** the test report is written to `playwright-report/`

#### Scenario: Suite passes in CI
- **WHEN** GitHub Actions runs the `test` job
- **THEN** the same five Playwright tests pass within CI's timeout
